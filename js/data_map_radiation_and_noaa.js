...
$.get('json/japan.geojson', function (geoJson) {
    ...
    // 获取节点数据
    $.when(
        $.get('json/data_map_radiation_and_noaa/noaa-node.json'),
        $.get('json/data_map_radiation_and_noaa/radiation-node.json')
    ).done(function (noaaNodeData, radiationNodeData) {
        // 处理节点数据
        var noaaNodeSeries = {
            name: 'Meteorological Monitoring Station',
            type: 'scatter',
        ...
        var radiationNodeSeries = {
            name: 'Radiation Monitoring Station',
            type: 'scatter',
            coordinateSystem: 'geo',
